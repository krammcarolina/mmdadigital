...
class Contatos {
  ...
  public  $_item    = array();
   ...
   $this->_item['responsavel'] = $row['responsavel'];
		}
		return $this->_item;
	}
	...
	public function getList($condition=NULL,$order_by_and_or_limit=NULL,$debug=NULL)  {		
	 global $db;
		...
		$SQL = "SELECT id, responsavel as text FROM contatos";
  ...
  for ($i = 0, $d = @mysql_num_rows($this->result); $i < $d; $i++) {
	...
	public function _updateDB($debug=NULL) 
	{
		...
		if($this->_item['id']!='')
		{
			$SQL = "UPDATE contatos SET ";
					if(!empty($this->_item['responsavel'])) {
						$SQL .= $virgula ." `responsavel`='".mysql_real_escape_string($this->_item['responsavel'])."'";				
						if($bfirstfield){
							$virgula=", ";
							$bfirstfield = false;
						}
	...
	public function createNewObject($debug=NULL) 
	{
	 global $db;
		...
		$SQL .= "(responsavel)";
		$SQL .= " VALUES (  
				'".mysql_real_escape_string($this->_item['responsavel']).",'
				)";
