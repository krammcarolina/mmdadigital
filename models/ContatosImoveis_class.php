...
class ContatosImoveis {
  private $_id          = null;
  private $_imovel      = null;
  private $_contato     = null;
  private $result;
  public  $_data        = array();
  public  $_item        = array();
	 ...
	 *
	 */
 	public function ContatosImoveis() {
 	  global $db;
	...
	public function getObject($id,$condition=NULL,$debug=NULL) {
		$this->_item=array();
		$SQL = "SELECT * FROM 'contatosimoveis' WHERE 'id' = '".mysql_real_escape_string($id)."' ";
		if($condition!=NULL) {
			$SQL .= " AND ". $condition;
		}
		$SQL .= "  LIMIT 1 ";
	 ...
	 * @return array
	 */
	public function getList($condition=NULL,$order_by_and_or_limit=NULL,$debug=0)  {		
	 global $db;
 
		$SQL  = "SELECT * FROM contatosimoveis";
		$SQL .= " INNER JOIN contatos
              ON  contatosimoveis.contato = contatos.id ";
  ...
  for ($i = 0, $d = @mysql_num_rows($this->result); $i < $d; $i++) {
	...
	public function _updateDB($debug=NULL) 
	{
		...
		if($this->_item['id']!='')
		{
			$SQL = "UPDATE contatosimoveis SET ";
			if(!empty($this->_item['imovel'])) {
				 $SQL .= $virgula ." `imovel`='".mysql_real_escape_string($this->_item['imovel'])."'";				
				 if($bfirstfield){
					  $virgula=", ";
					  $bfirstfield = false;
				 }
 		...
 		if(!empty($this->_item['contatos'])) {
				 $SQL .= $virgula ." `contatos`='".mysql_real_escape_string($this->_item['contatos'])."'";				
				 if($bfirstfield){
					  $virgula=", ";
					  $bfirstfield = false;
				 }
	...
	{
	 global $db;
		$SQL = "INSERT INTO contatosimoveis  ";
		$SQL .= "(imovel, contato)";
		$SQL .= " VALUES (  
				".mysql_real_escape_string($this->_item['imovel']).",
    ".mysql_real_escape_string($this->_item['contato'])."
				)";
