...
class FormasContatos {
  ...
  public	$_item         = array();
	...
	public function getObject($id,$condition=NULL,$debug=NULL) {
		$this->_item=array();
		$SQL = "SELECT * FROM 'formascontatos' WHERE 'id' = '".mysql_real_escape_string($id)."' ";
		if($condition!=NULL) {
			$SQL .= " AND ". $condition;
		}
		$SQL .= "  LIMIT 1 ";
  ...
  for ($i = 0, $d = @mysql_num_rows($this->result); $i < $d; $i++) {
	...
	public function _updateDB($debug=NULL) 
	{
		...
		if($this->_item['id']!='')
		{
			...
			if(!empty($this->_item['contatoid'])) {
				 $SQL .= $virgula ." `contatoid`='".mysql_real_escape_string($this->_item['contatoid'])."'";				
				 if($bfirstfield){
					  $virgula=", ";
					  $bfirstfield = false;
				 }
   ...
   if(!empty($this->_item['tipo'])) {
				 $SQL .= $virgula ." `tipo`='".mysql_real_escape_string($this->_item['tipo'])."'";				
				 if($bfirstfield){
					  $virgula=", ";
					  $bfirstfield = false;
				 }
   ...
   if(!empty($this->_item['contato'])) {
				 $SQL .= $virgula ." `contato`='".mysql_real_escape_string($this->_item['contato'])."'";				
				 if($bfirstfield){
					  $virgula=", ";
					  $bfirstfield = false;
				 }
	...
	public function createNewObject($debug=NULL) 
	{
	 global $db;
		...
		$SQL .= " VALUES (  
				'".mysql_real_escape_string($this->_item['contatoid']).",'
     ".mysql_real_escape_string($this->_item['tipo']).",
