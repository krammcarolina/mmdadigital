...
class Imoveis {
  ...
  public	$_item         = array();
	...
	public function getObject($id,$condition=NULL,$debug=NULL) {
		$this->_item=array();
		$SQL = "SELECT * FROM 'imoveis' WHERE 'id' = '".mysql_real_escape_string($id)."' ";
		if($condition!=NULL) {
			$SQL .= " AND ". $condition;
		}
		$SQL .= "  LIMIT 1 ";
  ...
  for ($i = 0, $d = @mysql_num_rows($this->result); $i < $d; $i++) {
	...
	public function _updateDB($debug=NULL) 
	{
		...
		if($this->_item['id']!='')
		{
			$SQL = "UPDATE imoveis SET ";
					...
					if(!empty($this->_item['tipoimovel'])) {
						$SQL .= $virgula ." `tipoimovel`='".mysql_real_escape_string($this->_item['tipoimovel'])."'";
						if($bfirstfield) {
							$virgula=", ";
							$bfirstfield = false;
						}
     ...
     if(!empty($this->_item['tipoanuncio'])) {
						$SQL .= $virgula ." `tipoanuncio`='".mysql_real_escape_string($this->_item['tipoanuncio'])."'";
						if($bfirstfield) {
							$virgula=", ";
							$bfirstfield = false;
						}
     ...
     if(!empty($this->_item['rua'])) {
						$SQL .= $virgula ." `rua`='".mysql_real_escape_string($this->_item['rua'])."'";
						if($bfirstfield) {
							$virgula=", ";
							$bfirstfield = false;
						}
     ...
     if(!empty($this->_item['numero'])) {
						$SQL .= $virgula ." `numero`='".mysql_real_escape_string($this->_item['numero'])."'";
						if($bfirstfield) {
							$virgula=", ";
							$bfirstfield = false;
						}
     ...
     if(!empty($this->_item['cidade'])) {
						$SQL .= $virgula ." `cidade`='".mysql_real_escape_string($this->_item['cidade'])."'";
						if($bfirstfield) {
							$virgula=", ";
							$bfirstfield = false;
						}
	...
	public function createNewObject($debug=NULL) 
	{
	 global $db;
		...
		$SQL .= " VALUES (  
				".mysql_real_escape_string($this->_item['tipoimovel']).",
    ".mysql_real_escape_string($this->_item['tipoanuncio']).",
    '".mysql_real_escape_string($this->_item['rua'])."',
    '".mysql_real_escape_string($this->_item['numero'])."',
    '".mysql_real_escape_string($this->_item['cidade'])."',
    '".mysql_real_escape_string($this->_item['bairro'])."',
    ".mysql_real_escape_string($this->_item['estado']).",
    '".mysql_real_escape_string($this->_item['descricao'])."'
		)";
