...
class Imagens {
  ...
  public	$_item       = array();
	...
	public function getObject($id,$condition=NULL,$debug=NULL) {
		$this->_item=array();
		$SQL = "SELECT * FROM 'imagens' WHERE 'id' = '".mysql_real_escape_string($id)."' ";
		if($condition!=NULL) {
			$SQL .= " AND ". $condition;
		}
		$SQL .= "  LIMIT 1 ";
  ...
  for ($i = 0, $d = @mysql_num_rows($this->result); $i < $d; $i++) {
		...
		if($this->_item['id']!='')
		{
			$SQL = "UPDATE imagens SET ";
					...
					if(!empty($this->_item['imagem'])) {
						$SQL .= $virgula ." `imagem`='".mysql_real_escape_string($this->_item['imagem'])."'";
						if($bfirstfield) {
							$virgula=", ";
							$bfirstfield = false;
						}
    ...
    '".mysql_real_escape_string($this->_item['imagem'])."'
		)";
